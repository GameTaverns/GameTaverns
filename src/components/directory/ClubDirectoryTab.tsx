...
import {
  Search, Users, ArrowRight, Building2, Globe, Plus, Ticket, MapPin,
} from "lucide-react";
...
export function ClubDirectoryTab() {
  const { t } = useTranslation();
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sub-tabs: All / Near Me */}
      <Tabs value={subTab} onValueChange={setSubTab}>
        <TabsList>
          <TabsTrigger value="all" className="gap-1">
            <Building2 className="h-4 w-4" /> {t('clubDirectory.allClubs')}
          </TabsTrigger>
          <TabsTrigger value="near-me" className="gap-1">
            <MapPin className="h-4 w-4" /> {t('clubDirectory.nearMe')}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="near-me" className="mt-4">
          <div className="text-center py-16">
            ...
            <h2 className="font-display text-xl text-foreground mb-2">
              {t('clubDirectory.nearMeTitle')}
            </h2>
            <p className="text-muted-foreground text-sm mb-6">
              {t('clubDirectory.nearMeDesc')}
            </p>
          </div>
        </TabsContent>
      </Tabs>
