...
AS $function$
  ...
  my_mechanics AS (
    SELECT DISTINCT mechanic_id FROM (
  ...
  mechanic_overlap AS (
    SELECT other_user_id, COUNT(DISTINCT mechanic_id) AS shared_count
    FROM (
      SELECT ol.other_user_id, gm.mechanic_id
      FROM game_mechanics gm
      JOIN games g ON g.id = gm.game_id
      JOIN other_libraries ol ON ol.library_id = g.library_id
      JOIN my_mechanics mm ON mm.mechanic_id = gm.mechanic_id
      UNION
      SELECT ol.other_user_id, cm.mechanic_id
      FROM catalog_mechanics cm
      JOIN games g ON g.catalog_id = cm.catalog_id
      JOIN other_libraries ol ON ol.library_id = g.library_id
      JOIN my_mechanics mm ON mm.mechanic_id = cm.mechanic_id
    ) combined
