...
-- =============================================================================
-- Designers & Artists tables + junction tables
-- =============================================================================
...
-- Catalog Designers junction (like catalog_mechanics)
...
-- Catalog Artists junction (like catalog_mechanics)
...
-- Game Designers junction (like game_mechanics)
...
CREATE POLICY "Library owners can insert game_designers" ON public.game_designers FOR INSERT
    WITH CHECK (
        EXISTS (SELECT 1 FROM games g JOIN libraries l ON l.id = g.library_id WHERE g.id = game_designers.game_id AND l.owner_id = auth.uid())
...
-- Game Artists junction (like game_mechanics)
...
CREATE POLICY "Library owners can insert game_artists" ON public.game_artists FOR INSERT
    WITH CHECK (
        EXISTS (SELECT 1 FROM games g JOIN libraries l ON l.id = g.library_id WHERE g.id = game_artists.game_id AND l.owner_id = auth.uid())
        OR has_role(auth.uid(), 'admin')
    );
