...
-- Trigger: Emit activity event when a game is added
...
CREATE TRIGGER trg_activity_game_added
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.emit_activity_game_added();

-- Trigger: Emit activity event when a session is logged
...
CREATE TRIGGER trg_activity_session_logged
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.emit_activity_session_logged();

-- Trigger: Emit activity event when an achievement is earned
...
BEGIN
  ...
  VALUES (
    ...
    'achievement_earned',
    jsonb_build_object(
      'name', COALESCE(ach_name, 'Achievement'),
      'icon', COALESCE(ach_icon, '🏆'),
...
CREATE TRIGGER trg_activity_achievement_earned
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.emit_activity_achievement_earned();

-- Trigger: Emit activity event when a forum thread is created
...
CREATE TRIGGER trg_activity_forum_post
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.emit_activity_forum_post();

-- Trigger: Emit activity event when a library is created
...
CREATE TRIGGER trg_activity_library_created
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.emit_activity_library_created();
