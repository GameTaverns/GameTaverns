...
-- Update the library_settings_public view to expose sync status (read-only, non-sensitive)
-- The full sync config is only readable by the library owner via the library-settings edge function
