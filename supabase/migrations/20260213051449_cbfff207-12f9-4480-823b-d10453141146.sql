...
CREATE INDEX idx_game_catalog_bgg_id ON public.game_catalog(bgg_id);
...
CREATE TABLE public.catalog_mechanics (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.catalog_publishers (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    catalog_id UUID NOT NULL REFERENCES public.game_catalog(id) ON DELETE CASCADE,
...
CREATE INDEX idx_catalog_videos_catalog ON public.catalog_videos(catalog_id);

-- Video votes (one vote per user per video)
CREATE TABLE public.catalog_video_votes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_catalog_corrections_status ON public.catalog_corrections(status);
...
CREATE POLICY "Anyone can view catalog" ON public.game_catalog
    FOR SELECT USING (true);

CREATE POLICY "Admins can manage catalog" ON public.game_catalog
...
CREATE POLICY "Anyone can view catalog mechanics" ON public.catalog_mechanics
    FOR SELECT USING (true);

CREATE POLICY "Admins can manage catalog mechanics" ON public.catalog_mechanics
    FOR ALL USING (has_role(auth.uid(), 'admin'::app_role))
    WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

-- catalog_publishers: public read
CREATE POLICY "Anyone can view catalog publishers" ON public.catalog_publishers
    FOR SELECT USING (true);

CREATE POLICY "Admins can manage catalog publishers" ON public.catalog_publishers
    FOR ALL USING (has_role(auth.uid(), 'admin'::app_role))
    WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

-- catalog_videos: public read, auth insert, admin manage
CREATE POLICY "Anyone can view catalog videos" ON public.catalog_videos
    FOR SELECT USING (true);

CREATE POLICY "Authenticated users can submit videos" ON public.catalog_videos
    FOR INSERT WITH CHECK (auth.uid() = submitted_by);

CREATE POLICY "Admins can manage catalog videos" ON public.catalog_videos
    FOR ALL USING (has_role(auth.uid(), 'admin'::app_role))
    WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

-- catalog_video_votes: auth users only
CREATE POLICY "Users can view their own votes" ON public.catalog_video_votes
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can vote on videos" ON public.catalog_video_votes
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can change their vote" ON public.catalog_video_votes
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can remove their vote" ON public.catalog_video_votes
    FOR DELETE USING (auth.uid() = user_id);

-- catalog_corrections: auth submit, admin review
CREATE POLICY "Users can view their own corrections" ON public.catalog_corrections
    FOR SELECT USING (auth.uid() = submitted_by);

CREATE POLICY "Users can submit corrections" ON public.catalog_corrections
    FOR INSERT WITH CHECK (auth.uid() = submitted_by);

CREATE POLICY "Admins can manage corrections" ON public.catalog_corrections
    FOR ALL USING (has_role(auth.uid(), 'admin'::app_role))
    WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

-- View: catalog with ownership count across libraries
CREATE OR REPLACE VIEW public.catalog_popularity AS
SELECT
    gc.id AS catalog_id,
    gc.title,
    gc.slug,
