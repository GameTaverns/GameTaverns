...
-- Replace with: event organizers, the attendee, and admins can view
...
-- Public access should go through library_settings_public view
...
-- 7. EVENT MANAGEMENT TABLES: Restrict writes to event organizers/library owners
...
-- Allow authenticated users to read event supplies (for claiming)
...
-- Keep anon read for event_games (public event pages need it)
-- (Already exists: "Anyone can view event games")
...
-- 8. STORAGE: Feedback attachments — require auth for upload, admin for read
...
-- 9. STORAGE: User photos — enforce path scoping on INSERT
...
-- Service role full access for all tables needing backend operations
DO $$
DECLARE
  tbl text;
BEGIN
