...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Validate input
    const allowedFields = ['display_name', 'username', 'bio', 'avatar_url', 'featured_achievement_id'];
    ...
    if (updates.featured_achievement_id !== undefined && updates.featured_achievement_id !== null) {
      ...
      if (!hasAchievement) {
        return new Response(JSON.stringify({ error: 'You can only feature achievements you have earned' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
