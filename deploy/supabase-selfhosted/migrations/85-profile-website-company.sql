-- =============================================================================
-- Profile Website & Company Fields
-- =============================================================================
...
-- Recreate public_user_profiles view to include new fields
...
LEFT JOIN LATERAL (
    SELECT count(*) FILTER (WHERE NOT g.is_expansion) AS games_owned,
...
LEFT JOIN LATERAL (
    ...
    FROM game_sessions gs JOIN games g ON g.id = gs.game_id JOIN libraries l ON l.id = g.library_id
    WHERE l.owner_id = up.user_id
) session_stats ON true
LEFT JOIN LATERAL (
    SELECT count(*) AS achievements_earned, COALESCE(sum(a.points), 0::bigint) AS achievement_points
...
-- Recreate user_profiles_minimal to include new fields
DROP VIEW IF EXISTS public.user_profiles_minimal;
CREATE VIEW public.user_profiles_minimal AS
SELECT up.user_id, up.display_name, up.username, up.avatar_url, up.website_url, up.company_name
FROM user_profiles up
JOIN libraries l ON l.owner_id = up.user_id
WHERE l.is_active = true;

GRANT SELECT ON public.user_profiles_minimal TO anon, authenticated;
