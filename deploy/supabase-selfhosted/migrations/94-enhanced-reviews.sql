-- Migration 94: Enhanced review system - rating tags, player count ratings, guided prompts
...
DROP POLICY IF EXISTS "Rating tags are publicly readable" ON public.rating_tags;
...
DROP POLICY IF EXISTS "Review tags are publicly readable" ON public.review_tags;
CREATE POLICY "Review tags are publicly readable" ON public.review_tags FOR SELECT USING (true);
DROP POLICY IF EXISTS "Users can manage own review tags" ON public.review_tags;
...
DROP POLICY IF EXISTS "Users can delete own review tags" ON public.review_tags;
...
CREATE INDEX IF NOT EXISTS idx_review_pc_ratings_review ON public.review_player_count_ratings(review_id);

ALTER TABLE public.review_player_count_ratings ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Player count ratings are publicly readable" ON public.review_player_count_ratings;
CREATE POLICY "Player count ratings are publicly readable" ON public.review_player_count_ratings FOR SELECT USING (true);
DROP POLICY IF EXISTS "Users can manage own player count ratings" ON public.review_player_count_ratings;
CREATE POLICY "Users can manage own player count ratings" ON public.review_player_count_ratings FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.game_reviews gr WHERE gr.id = review_id AND gr.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can delete own player count ratings" ON public.review_player_count_ratings;
